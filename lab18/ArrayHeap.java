...
public class ArrayHeap<T> {
    ...
    private void bubbleUp(int index) {
        if (index > 1 && min(index, getParentOf(index)) == index) {
            swap(index, getParentOf(index));
            //bubbleDown(getParentOf(index));
	...
	private void bubbleDown(int index) {
		Boolean hasLeft = getLeftOf(index) < contents.size(),
				hasRight = getRightOf(index) < contents.size(),
				leftIsBigger = hasLeft && min(index, getLeftOf(index)) == index,
				rightIsBigger = hasRight && min(index, getRightOf(index)) == index;

		if (hasLeft && hasRight && !leftIsBigger && !rightIsBigger) {
			int minIndex = min(getLeftOf(index), getRightOf(index));
			swap(index, minIndex);
			bubbleDown(minIndex);
		} else if (hasRight && !rightIsBigger) {
			swap(index, getRightOf(index));
			bubbleDown(getRightOf(index));
		} else if (hasLeft && !leftIsBigger) {
