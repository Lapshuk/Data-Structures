...
public class SplayTree<K extends Comparable<K>, V> {
    ...
    void rotateRight(TreeNode<K, V> node) {
        ...
        //repoint things related to grandparent
        if (grandParent == null) {
            root = node;
    ...
    private void zig(TreeNode<K, V> node) {
        // if x is a child of the root, we rotate x through p so that x becomes the root.
        TreeNode<K, V> parent = node.parent;
        if (parent == root) {
    ...
    private void zigZag(TreeNode<K, V> node) {
        //if x is the right child of a left child, we first rotate x left
        // through p and then rotate x right through g.
        //The symmetric case where x is the left child of a
        // right child uses the same rotations in mirror image:
        // first rotate x right through p and then rotate left through g.
        TreeNode<K, V> parent = node.parent;
        TreeNode<K, V> grandParent = node.parent.parent;
