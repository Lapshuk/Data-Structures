...
public class IntList {
    ...
    private IntListNode insert(IntListNode p, IntListNode head) {
        if (head == null) {
            return new IntListNode(p.item);
        }

        IntListNode curr = head;
        for (; curr.item < p.item; curr = curr.next) {
            if (curr.next == null) {
                curr.next = new IntListNode(p.item, curr, null);
                return head;
            }
        }
        IntListNode newNode = new IntListNode(p.item, curr.prev, curr);
        if (curr.prev == null) {
            head = newNode;
        } else {
    ...
    public IntList quicksort() {
        ...
        for (IntListNode i = head; i != null; i = i.next) {
            if (i.item < divider) {
                smallElements.addToEnd(i.item);
            } else {
                largeElements.addToEnd(i.item);
            }
        }

        IntList sorted = smallElements.quicksort();
        sorted.addToEnd(divider);
    ...
    public IntList mergeSort() {
        ...
        for (int i = 0; i < size; i++, head = head.next) {
            if (i >= size/2) {
                otherHalf.addToEnd(head.item);
            } else {
                oneHalf.addToEnd(head.item);
            }
        }
